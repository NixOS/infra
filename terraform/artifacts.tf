...
# - /nix/tag/* -> /NixOS/experimental-nix-installer/releases/download/*
# - /nix -> /NixOS/experimental-nix-installer/releases/latest/download/nix-installer.sh
# - /nix/* -> /NixOS/experimental-nix-installer/releases/latest/download/*
  ...
  snippet {
    content  = <<-EOT
      ...
      if (!req.http.X-Rewritten) {
        if (req.url ~ "^/nix/tag/") {
          set req.url = regsub(req.url.path, "^/nix/tag/", "/NixOS/experimental-nix-installer/releases/download/");
          set req.http.X-Rewritten = "true";
        } else if (req.url ~ "(/nix|/nix/)") {
          set req.url = regsub(req.url.path, "^/nix$", "/NixOS/experimental-nix-installer/releases/latest/download/nix-installer.sh");
          set req.http.X-Rewritten = "true";
        } else if (req.url ~ "^/nix/") {
          set req.url = regsub(req.url.path, "^/nix", "/NixOS/experimental-nix-installer/releases/latest/download/");
          set req.http.X-Rewritten = "true";
